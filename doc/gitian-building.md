...
*Setup instructions for a Gitian build of Adn Core using a Debian VM or physical system.*
...
- [Building Adn Core](#building-adn-core)
...
Clone the git repositories for Adn Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/adnpay/adn
```
...
in the Adn Core repository under 'Fetch and build inputs' to install sources which require
...
To build Adn Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Adn Core repository.
...
```bash
    ...
    From https://github.com/adnpay/adn
...
and the adn git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../adn/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/adn.git
SIGPATH=/some/root/path/adn-detached-sigs.git

./bin/gbuild --url adn=${BTCPATH},signature=${SIGPATH} ../adn/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/adn-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/adn-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/adn-osx-build.assert
```
...
[adn/gitian.sigs](https://github.com/adnpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
