...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Adn address (e.g. %1)").arg("XwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwg"));
#endif
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("adn://", Qt::CaseInsensitive))
    {
        uri.replace(0, 7, "adn:");
    }
...
{
    QString ret = QString("adn:%1").arg(info.address);
    int paramCount = 0;
...
boost::filesystem::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Adn (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Adn (%s).lnk", chain);
}
...
boost::filesystem::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("adn-%s.lnk", chain);
}
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Adn Core app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Adn Core app to startup item list
