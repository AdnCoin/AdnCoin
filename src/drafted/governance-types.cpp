...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "AdnNetwork") type = AdnNetwork;
    if(strType == "AdnNetworkVariable") type = AdnNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == AdnNetwork) s = "AdnNetwork";
    if(type == AdnNetworkVariable) s = "AdnNetworkVariable";
